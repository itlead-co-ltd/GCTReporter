...
| US002 | Session管理 | P0 | 1天 | B | ⚪ 待开始 |
| US003 | 数据库初始化 | P0 | 0.5天 | B | ⚪ 待开始 |
...
#### T002-4: 退出登录接口（开发者B，0.5h）
**描述**: 实现POST /api/v1/auth/logout接口
**验收标准**:
- [ ] 调用SessionService.destroySession
- [ ] 清除Session
- [ ] 返回200成功

#### T002-5: 前端退出登录（开发者A，0.5h）
**描述**: 实现退出登录功能
**验收标准**:
- [ ] 退出按钮调用logout API
