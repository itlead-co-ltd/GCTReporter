...
#### 迭代计划文档（docs/Sprint{N}-迭代任务计划.md）

**🔒 文档保护规则**:

- ❌ **禁止修改**：`docs/Sprint{N}-迭代任务计划.md`（如Sprint1、Sprint2、Sprint3...）是Sprint规划的基准文档，一旦Sprint启动后**不允许修改**
- ✅ **作为参考**：作为任务执行的参考依据，保持计划的稳定性和可追溯性
- ✅ **仅查阅**：开发过程中仅用于查阅Story和Task定义，不做任何更改
...
**📝 文档命名规则**:
- Sprint 1: `docs/Sprint1-迭代任务计划.md`
- Sprint 2: `docs/Sprint2-迭代任务计划.md`
- Sprint 3: `docs/Sprint3-迭代任务计划.md`
- 以此类推...

### 项目状态文档管理

#### 项目状态文档（PROJECT_STATUS_v{X}.{Y}.md）

**📝 更新规则**:

1. **Story完成后更新** - 当某个Story的所有Task执行完成后，必须更新当前Sprint的项目状态文档：
   - [ ] 更新Story状态为✅已完成
   - [ ] 更新所有Task状态和实际工时
   - [ ] 记录Story完成时间
   - [ ] 补充"最近完成的任务明细"章节（核心成果、技术实现、验收达成等）
   - [ ] 补充"Sprint开发日志"章节（执行时间序列、关键动作）
   - [ ] 更新Sprint整体进度和工时统计

2. **Task进行中更新** - 当开始执行某个Task时：
   - [ ] 更新Story状态为🔧进行中（如果是该Story的第一个Task）
   - [ ] 更新Task状态为🔧进行中
   - [ ] 可选：记录Task开始时间

3. **Task完成后更新** - 当某个Task执行完成后：
   - [ ] 更新Task状态为✅已完成
   - [ ] 记录Task实际工时和完成时间
   - [ ] 如果是Story的最后一个Task，按"Story完成后更新"规则处理

4. **同步更新README.md**：
   - [ ] 每次Story完成后，同步更新项目根目录的README.md
   - [ ] 更新项目整体进度、完成的功能列表
   - [ ] 更新最近更新日期

**📝 文档命名规则**:
- Sprint 1进度跟踪: `PROJECT_STATUS_v1.0.md`
- Sprint 2进度跟踪: `PROJECT_STATUS_v2.0.md`
- Sprint 3进度跟踪: `PROJECT_STATUS_v3.0.md`
- Sprint完成归档: `Sprint-{N}-Complete-Status.md`

**⚠️ 关键提醒**:
- 项目状态文档是**动态文档**，必须随任务进展实时更新
- Sprint迭代计划是**静态基准**，保持不变作为对比参照

### 开发日志管理规范

**🎯 开发日志的重要性**:

开发日志是**项目上下文管理的核心**，记录了：
- ✅ 每个任务的执行时间序列
- ✅ 关键决策和解决方案
- ✅ 技术难点和突破过程
- ✅ 实际工时和效率分析
...
1. **Story开始时**：创建日志框架
2. **执行过程中**：记录关键决策点和问题解决
3. **Story完成时**：补全完整的执行序列和成果总结
...
- 📝 每个Story完成后**立即**更新文档，不要积压
- 🔍 开发日志要记录**真实**的执行过程，不是事后编造
- 📊 工时记录要**准确**，反映真实耗时
- 🔄 README要与项目状态文档**保持同步**
...
**维护人**: 技术组
**变更记录**: 
- v1.1 (2026-01-15): 新增"项目管理规范"章节，包含迭代计划管理、项目状态文档管理、开发日志管理规范
